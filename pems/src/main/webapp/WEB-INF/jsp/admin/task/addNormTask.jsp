...
<script src="${ctx }/resources/js/admin/task/normTask.js"></script>
	...
	<form class="form-horizontal" role="form" id="editForm">
		...
		<%-- <div class="form-group">
			<label for="disabledSelect" class="col-sm-2 control-label"><b class="z_common_star">*</b>模板</label>
			<div class="col-sm-10">
			    <c:forEach var="entity" items="${NormTemplateList }">
			         <input type="radio" name="normTempId"  value="${entity.normTempId }" <c:if test="${entity.normTempId==normTask.normTempId }">checked="checked"</c:if>/>${entity.normTempName }
			    </c:forEach>
			</div>
		</div> --%>
		<div class="form-group">
			...
			<div class="col-sm-10">
				<input class="datainp wicon" id="inpstartTime" type="text" placeholder="考核开始时间" name="startTime" readonly> 
			</div>
		</div>
		<div class="form-group">
			<label for="disabledSelect" class="col-sm-2 control-label"><b class="z_common_star">*</b>考核结束时间</label>
			<div class="col-sm-10">
				<input class="datainp wicon" id="inpendTime" type="text" placeholder="考核结束时间" name="endTime"  readonly>
			</div>
		</div>
		<div class="form-group">
			<label for="disabledSelect" class="col-sm-2 control-label"><b class="z_common_star">*</b>评分截止时间</label>
			<div class="col-sm-10">
				<input class="datainp wicon" id="inpstart" type="text" placeholder="请选择考核归档时间" name="scoreTime" readonly> 
			</div>
		</div>
		...
		<div class="form-group">
			...
			<select class="form-control combobox input-sm s_sel" name="normTempId" id="combobox">
			...
			<input type="hidden" class="normTemp" name="normTempId" value="@">
		</div>
		</c:forEach>
		<%-- <div class="form-group">
		<label for="disabledSelect" class="col-sm-2 control-label"><b class="z_common_star">*</b>选择岗位</label>
		<div class="col-sm-10">
		<select class="form-control combobox input-sm" id="combobox">
			<option value="-1"></option>
			<c:forEach var="entity2" items="${stationList }">
			     <option value="${entity2.stationId}">${entity2.stationName }</option>
			</c:forEach>
		</select>
	    </div>
		</div> --%>
